...
export function createWorldContext(manifest = {}, meta = {}) {
  ...
  const locations = arr(map.locations)
    .map((loc) => ({
      name: str(loc.name),
  ...
  const skillTree = arr(manifest.skillTree)
    .map((skill) => ({
