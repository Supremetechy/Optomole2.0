/**
 * BootScene (idle-progress) — title card + how-to shown before the lab opens.
 *
 * Shows the experience title, the coach briefing (folded in from any NPC
 * binding), and the idle loop rules, then waits for the first gesture
 * (click / tap / E / Space) — which also unlocks audio.
 */
...
export class BootScene {
  constructor(ctx, { title, subtitle, briefing, onStart }) {
    ...
    this._title = title || 'Optomole Idle';
    this._subtitle = subtitle || 'Idle · Progress';
    this._briefing = briefing || 'Invest Insight into each concept to automate your learning, then push every milestone to full mastery.';
    this._t = 0;
  }
  ...
  _layout({ width, height }) {
    ...
    const rules = [
      [0x67e8f9, 'Click STUDY (or E) to earn Insight'],
      [0x34d399, 'Buy concepts — each auto-produces Insight'],
      [0xa78bfa, 'Buy upgrades and hit every milestone'],
    ];
    rules.forEach((r, i) => {
