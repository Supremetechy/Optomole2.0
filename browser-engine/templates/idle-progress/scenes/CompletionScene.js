/**
 * CompletionScene (idle-progress) — the mastery / victory screen.
 *
 * Shown once every concept is unlocked and every milestone is met. Fires a
 * celebratory burst and summarizes the run (lifetime Insight, concepts unlocked,
 * upgrades bought, production rate), then offers a fresh start.
 */
...
export class CompletionScene {
  constructor(ctx, { economy, onReplay }) {
    this.ctx = ctx;
  ...
  _layout({ width, height }) {
    ...
    const kicker = text('FULLY MASTERED', { fill: 0x34d399, fontSize: 13, fontWeight: '800', letterSpacing: 2 });
    kicker.anchor.set(0.5);
    kicker.position.set(cx, height / 2 - 118);
    this.ui.addChild(kicker);

    const title = text('Every Concept Automated', { fill: 0xffffff, fontSize: 26, fontWeight: '900', align: 'center', wordWrap: true, wordWrapWidth: pw - 60 });
    title.anchor.set(0.5);
    title.position.set(cx, height / 2 - 82);
    this.ui.addChild(title);

    const stats = [
      ['Lifetime Insight', fmt(e.lifetime)],
      ['Concepts Unlocked', `${e.ownedGeneratorCount()}/${e.generators.length}`],
      ['Upgrades Bought', `${e.upgrades.filter((u) => u.bought).length}/${e.upgrades.length}`],
      ['Insight / sec', fmt(e.ratePerSec())],
    ];
    stats.forEach((row, i) => {
      const col = i % 2;
    ...
    btnBg.roundRect(cx - 130, height / 2 + 108, 260, 44, 22).fill(0x34d399);
    this.ui.addChild(btnBg);
    const btn = text('↻  START OVER  (E)', { fill: 0x03131d, fontSize: 15, fontWeight: '900' });
    btn.anchor.set(0.5);
