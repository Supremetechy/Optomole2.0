/**
 * BootScene (arcade-collect-avoid) — title card + how-to shown before play.
 *
 * Displays the experience title, a one-line coach briefing (folded in from any
 * NPC binding in the manifest), and the collect/avoid rules, then waits for the
 * first gesture (click / tap / E / Space) — which also unlocks audio.
 */
...
export class BootScene {
  constructor(ctx, { title, subtitle, briefing, onStart }) {
    ...
    this._title = title || 'Optomole Arcade';
    this._subtitle = subtitle || 'Arcade · Collect & Avoid';
    this._briefing = briefing || 'Grab the correct concepts, dodge the decoys, and clear every wave before the clock runs out.';
    this._t = 0;
  }
  ...
  _layout({ width, height }) {
    ...
    // Rules legend: green = collect, red = avoid, star = power-up.
    const rules = [
      [0x34d399, 'Collect the labelled orbs'],
      [0xfb7185, 'Avoid the red decoys'],
      [0xa78bfa, 'Grab power-ups for a shield + ×2 score'],
    ];
    rules.forEach((r, i) => {
