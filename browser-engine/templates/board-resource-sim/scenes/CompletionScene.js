/**
 * CompletionScene (board-resource-sim) — the victory screen.
 *
 * Shown once every concept square has been collected. Fires a celebratory burst
 * and summarizes the run (Knowledge, concepts, laps, turns, coins), then offers
 * a fresh board.
 */
const PIXI = window.PIXI;

export class CompletionScene {
  constructor(ctx, { model, onReplay }) {
    this.ctx = ctx;
    this.model = model;
  ...
  _layout({ width, height }) {
    ...
    const kicker = text('BOARD COMPLETE', { fill: 0x34d399, fontSize: 13, fontWeight: '800', letterSpacing: 2 });
    kicker.anchor.set(0.5);
    kicker.position.set(cx, height / 2 - 118);
    this.ui.addChild(kicker);

    const title = text('Every Square Mastered', { fill: 0xffffff, fontSize: 26, fontWeight: '900', align: 'center', wordWrap: true, wordWrapWidth: pw - 60 });
    ...
    const stats = [
      ...
      ['Concepts', `${m.collected.size}/${m.conceptCount}`],
      ['Laps', m.laps],
      ['Turns Taken', m.turns],
    ];
    stats.forEach((row, i) => {
    ...
    const btn = text('↻  NEW BOARD  (E)', { fill: 0x03131d, fontSize: 15, fontWeight: '900' });
    btn.anchor.set(0.5);
